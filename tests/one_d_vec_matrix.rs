...
fn test_new() {
    ...
    let matrix = OneDVecMatrix::new(data.clone());

    assert_eq!(matrix.shape(), (2, 2));
    assert_eq!(matrix.num_rows(), 2);
    assert_eq!(matrix.num_cols(), 2);
    assert_eq!(*matrix.get_data_vec_vec(), data);
}

#[test]
fn test_zeroes() {
    let matrix = OneDVecMatrix::zeroes(2, 3);
    ...
    for row in matrix.get_data_vec_vec() {
        for val in row {
            assert_eq!(val, 0.0);
        }
    }
...
#[test]
fn test_identity() {
    let matrix = OneDVecMatrix::identity(3);
...
fn test_matrix_multiplication_identity() {
    ...
    let matrix = OneDVecMatrix::new(data.clone());
    let identity = OneDVecMatrix::identity(3); // Identity matrix for multiplication

    let result = matrix.matrix_multiplication(&identity);

    // Should be unchanged after multiplying by identity
    assert_eq!(result.get_data_vec_vec(), data);
}

#[test]
fn test_matrix_addition() {
    let matrix1 = OneDVecMatrix::new(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
    let matrix2 = OneDVecMatrix::new(vec![vec![5.0, 6.0], vec![7.0, 8.0]]);

    let result = matrix1.matrix_addition(&matrix2);

    // Check result
    assert_eq!(
        result.get_data_vec_vec(),
...
fn test_matrix_subtraction() {
    let matrix1 = OneDVecMatrix::new(vec![vec![5.0, 6.0], vec![7.0, 8.0]]);
    let matrix2 = OneDVecMatrix::new(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
...
fn test_matrix_multiplication() {
    let matrix1 = OneDVecMatrix::new(vec![vec![1.0, 2.0, 3.0], vec![4.0, 5.0, 6.0]]);
    let matrix2 = OneDVecMatrix::new(vec![vec![7.0, 8.0], vec![9.0, 10.0], vec![11.0, 12.0]]);
...
fn test_scalar_multiplication() {
    let matrix = OneDVecMatrix::new(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
    let scalar = 2.0;
